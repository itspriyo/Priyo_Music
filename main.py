    ...
    if Config.DATABASE_URI:
        try:
            if await db.is_saved("RESTART"):
                ...
                if msg:
                    try:
                        k=await bot.edit_message_text(msg['chat_id'], msg['msg_id'], text="ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ʀᴇꜱᴛᴀʀᴛᴇᴅ.")
                        await db.del_config("RESTART")
                    except:
                        pass
            await sync_from_db()
        except Exception as e:
            LOGGER.error(f"Errors occured while setting up database for PremiumMusic_Bot, check the value of DATABASE_URI. Full error - {str(e)}", exc_info=True)
            Config.STARTUP_ERROR="Errors occured while setting up database for PremiumMusic_Bot, check the value of DATABASE_URI. Full error - {str(e)}"

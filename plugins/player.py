    ...
    with suppress(MessageIdInvalid, MessageNotModified):
        ...
        if Config.ADMIN_ONLY:
            if not (message.from_user is None and message.sender_chat or message.from_user.id in admins):
                k=await message.reply_sticker("CAACAgUAAx0CXDcUigACGxdhZSJcuiHb07fmtqPhbS7ThNHZkgACLwMAAtIz6VQWAAEQUgm7sCweBA")
        ...
        if message.reply_to_message and message.reply_to_message.video:
            ...
            await msg.edit("💗 **ᴄʜᴇᴄᴋɪɴɢ ᴛᴇʟᴇɢʀᴀᴍ ᴍᴇᴅɪᴀ...**")
...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAACAgUAAx0CXDcUigACGxdhZSJcuiHb07fmtqPhbS7ThNHZkgACLwMAAtIz6VQWAAEQUgm7sCweBA", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⚡️Join Here', url='https://t.me/Premium_Mirror')]]))
    await delete_messages([m, k])
